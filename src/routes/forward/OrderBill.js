...
function OrderBill({ location, dispatch, orderBill }) {
  ...
  const batchProcessBookRegOrderBillsProps = {
    ...
    records: bookRegOrderBillEntitys || [],
    next: orderBillNext,
    actionText: '预约',
    entityCaption: '入库订单',
  ...
  const batchProcessFinishOrderBillsProps = {
    ...
    records: finishOrderBillEntitys || [],
    next: orderBillNext,
    actionText: '完成',
    entityCaption: '入库订单',
  ...
  const orderBillItemsProps = {
    ...
    editable: !showViewPage,
    inProgressBill: currentItem.state === 'InProgress',
    articleQpcs,
