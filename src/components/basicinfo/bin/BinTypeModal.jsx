            ...
            render: (text, record) => {
                return (
                    <div className={styles.editable_row_operations}>
                        {record.editable ?
                            <span>
                                <a onClick={() => { onSave(record) }} disabled={!PermissionUtil("bin:createType")}>保存</a>
                            ...
                            :
                            <span>
                                <a onClick={() => { onEdit(record) }} disabled={!PermissionUtil("bin:createType")}>编辑</a>
                                <Popconfirm title={"确定要删除吗？"} onConfirm={() => { onDelete(record) }}>
                                    <a disabled={!PermissionUtil("bin:createType")}>删除</a>
                                </Popconfirm>
                            </span>
                        }
                    </div>
