...
import RtnSupplierNtcBillCreateItem from './RtnSupplierNtcBillCreateItem';
...
const RtnSupplierNtcBillCreateForm = ({
    ...
    supplier,
    onEnterSupplier,
    RtnSupplierNtcBillCreateItemProps,
}) => {
  ...
  function handleEnterPressSupplier() {
    if (getFieldsValue().supplier == null || getFieldsValue().supplier == '') { return; }
    const data = {
  ...
  children.push(<BaseFormItem label={'供应商：'} key="supplier">
    {getFieldDecorator('supplier', {
      ...
      <Input
        placeholder="请选择" suffix={<Icon type="ellipsis" onClick={() => onSupplierSelect()} />}
