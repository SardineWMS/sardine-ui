...
const WaveBillView = ({
  ...
  function convertState(text) {
    ...
    if (text == 'inProgress') { return '启动中'; }
    if (text == 'exception') { return '启动异常'; }
    if (text == 'started') { return '启动完成'; }
    if (text == 'inAlc') { return '配货中'; }
    if (text == 'finished') { return '已完成'; }
    return ''
  }

  function convertType(text) {
    if (text == 'normal') { return '正常波次'; }
    if (text == 'eCommerce') { return '电商波次'; }
    return ''
  }
        ...
        <Tabs defaultActiveKey="1">
          ...
          {item.rplItems.length >= 1 ?
            <TabPane key="3" tab="补货单">
              <BaseForm items={<WaveBillPickupItem {...waveBillRplItem} />} />
            </TabPane> : <div />}
        </Tabs>
      </BaseCard>
